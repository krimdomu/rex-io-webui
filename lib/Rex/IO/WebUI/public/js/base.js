  ...
  $(document).ready(function() {
    ...
    $("li.has-sub").each(function(idx, elem) {
      $(elem).click(function(event) {
        var classes = $(event.srcElement).attr("class");
        // check if click is from mnu-item
        if(! classes.match(/mnu\-item/)) {
          console.log("no click from mnu-item");
          $(elem).trigger("mnuclick", [event.srcElement]);
          return;
        }
        if($(elem).find("ul.sub").css("display") == "none") {
          $(elem).find("ul.sub").show();
          $(elem).find("a").addClass("mnu-icon-arrow-down").removeClass("mnu-icon-arrow-left");
        }
        else {
          $(elem).find("ul.sub").hide();
          $(elem).find("a").addClass("mnu-icon-arrow-left").removeClass("mnu-icon-arrow-down");
        }
