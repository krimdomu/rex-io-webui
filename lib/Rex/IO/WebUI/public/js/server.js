...
(function() {
   ...
   $(document).ready(function() {
      ...
      $(".server-add-link").click(function(event) {
         ...
         load_page("/server/new", null, function() {
...
function list_server() {
   load_page("/server", null, function() {
...
function load_bulk_view(servers) {
   ...
   load_page("/server/bulk", null, function() {
...
function load_server(srv_id, callback) {
   ...
   load_page("/server/" + srv_id, null, function() {
...
function run_tasks_on_server(tasks, callback) {
   ...
   $("#run_services").button( "disable" );

   window.setTimeout(function() {
      $("#run_services").button( "enable" );
   }, 5000);
   ...
   $.ajax({
      ...
      if(data.ok != true) {
         ...
         $.pnotify({
            text: "Command(s) scheduled for server(s)",
            type: "info"
         });
      }
   });
   ...
   if(typeof ref.push == "function") {
      ...
      if(ref.cmd == "Answer") {
         ...
         if(log_div) {
            ...
            ref.line.replace(/INFO/, '<span class="green_text">INFO</span>');
            ref.line.replace(/WARN/, '<span class="orange_text">WARN</span>');
            ref.line.replace(/ERROR/, '<span class="red_text">ERROR</span>');
