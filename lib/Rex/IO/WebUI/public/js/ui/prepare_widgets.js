...
class_ui.implement({
  ...
  _prepare_ui_widgets_tree: function() {
    var self = this;
    $(".rexio-ui-tree").each(function() {
      console.log("tree is _initialized: " + $(this).attr("_initialized"));
      if($(this).attr("_initialized") == "true")
        return;
      
      self.tree(this);
      $(this).attr("_initialized", true);
    });
  },
  ...
  _prepare_ui_widgets_select_dropdown: function() {
    ...
    $(".rexio-ui-select").each(function() {
      var elem = this;
      var tmp;
      if($(elem).attr("rexio-ui-select-rows")) {
        // load data via ajax
        $.ajax(
          {
            "type": "GET",
            "url": $(elem).attr("rexio-ui-select-rows"),
          }
        ).done(function(data) {
          if(data['ok']) {
            var opt_a = new Array();
            for(var i = 0; i < data['data'].length; i++) {
              opt_a.push("<option value=\"" + data['data'][i]['id'] + "\">" + data['data'][i]['name'] + "</option>");
            }
            $(elem).html(opt_a.join("\n"));
          }
          else {
            throw "Error loading data for selectbox via ajax.";
          }
        });
      }
  ...
  _prepare_ui_widgets_data_table: function() {
    ...
    $(".rexio-ui-data-table").each(function(idx) {
      ...
      var searchable   = $(elem).attr("rexio-ui-searchable");
