      ...
      self._render_header(elem, $(elem).attr("rexio-ui-datatable-columns"), function() {
        if($(elem).attr("rexio-ui-datatable-rows")) {
          $.ajax(
            {
              "type": "GET",
              "url": $(elem).attr("rexio-ui-datatable-rows")
            }
          ).done(function(data) {
            if(data['ok']) {
              var tbl_tr = new Array();
              for(var i = 0; i < data['data'].length; i++) {
                tbl_tr.push("<tr>");
                for(var ii = 0; ii < data['data'][i].length; ii++) {
                  if($(elem).attr("rexio-ui-datatable-rows-on-render")) {
                    var tmp = $(elem).attr("rexio-ui-datatable-rows-on-render").split(/\./);
                    tbl_tr.push(self._objects[tmp[0]]()[tmp[1]](data['data'][i][ii], ii, data['data'][i]));
                  }
                  else {
                    tbl_tr.push("<td>" + data['data'][i][ii] + "</td>");
                  }
                }
                tbl_tr.push("</tr>");
              }
              $(elem).find("tbody").html(tbl_tr.join("\n"));
